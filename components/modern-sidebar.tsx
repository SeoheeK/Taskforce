...
// 진행 중인 프로젝트 목록 (실제로는 API에서 가져와야 함)
const activeProjects = [
  ...
  { id: 4, name: "Marketing Campaign", status: "review", icon: Eye },
  { id: 5, name: "API Integration", status: "completed", icon: CheckCircle },
]
...
const navigation = [
  ...
  },
  {
    ...
    subItems: [
      ...
      { name: "Prompt 템플릿", href: "/prompt-templates", icon: FileText, category: "AI 에이전트 설정" },
      { name: "MCP 전략", href: "/mcp-tasks", icon: Zap, category: "AI 에이전트 설정" },
      { name: "Output Format", href: "/output-formats", icon: BookOpen, category: "AI 에이전트 설정" },

      // PM 도구 (운영/관리자용)
      { name: "Workflow 템플릿", href: "/workflow-templates", icon: Workflow, category: "PM 도구" },
      { name: "평가표 템플릿", href: "/evaluation-templates", icon: BarChart3, category: "PM 도구" },
      { name: "프로젝트 템플릿", href: "/project-templates", icon: FolderOpen, category: "PM 도구" },
      { name: "산출물 템플릿", href: "/deliverable-templates", icon: ClipboardList, category: "PM 도구" },
      { name: "회의 유형/포맷", href: "/meeting-formats", icon: Calendar, category: "PM 도구" },
    ],
  },
      ...
      <div className="p-3">
        <Link href="/taskforce/new">
          ...
          >
            ...
            {isExpanded && <span className="ml-2">새 태스크포스</span>}
          </Button>
        </Link>
      </div>
